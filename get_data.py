...
batches = ["W20", "S19", "W19", "S18", "W18", "S17", "W17", "IK12","S16", "W16", "S15", "W15", "S14", "W14", "S13", "W13", "S12", "W12", "S11", "W11", "S10", "W10", "S09", "W09", "S08", "W08", "S07", "W07", "S06", "W06"]

for target_batch in batches:
    # Global semaphore to limit concurrent browser operations
    browser_semaphore = Semaphore(50)  # Limit to 3 concurrent browsers
    ...
    def process_url(url):
        with browser_semaphore:  # Use semaphore to limit concurrent browsers
                ...
                try:
                    ...
                    while retry_count < max_retries:
                        try:
                            ...
                            print(f"Browser error for {url} (attempt {retry_count + 1}/{max_retries}): {str(e)}")
                            retry_count += 1
                            if retry_count == max_retries:
                                all_companies_data.append({
                                    "yc_page_url": url,
                                    "error": f"Browser error after all retries: {str(e)}"
                                })
                            else:
    ...
    # Process URLs sequentially with controlled concurrency
    ...
    print(f"All URLs have been processed. Data saved to {output_filename}.")  # Indicate completion